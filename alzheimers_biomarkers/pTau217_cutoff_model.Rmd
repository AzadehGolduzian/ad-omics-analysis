---
title: "pTau217 Cut-Point Model for Aβ Positivity (Simulated Data)"
...
library(erikmisc)   # remotes::install_github("erikerhardt/erikmisc")
library(tibble)
library(mice)
library(missForest)
library(knitr)
...
## Scatter Plot: Observed Positivity vs pTau217
...
scatter_plot <- ggplot(clean_data, aes(
  ...
  geom_vline(
    xintercept = (log(thresh / (1 - thresh)) - coef(model)[1]) / coef(model)[2],
    linetype   = "dashed",
    color      = "blue"
  ) +
  labs(
...
```{r combined-plot, fig.width=12, fig.height=4}
